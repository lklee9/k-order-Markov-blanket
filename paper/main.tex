...
\begin{lemma}[$k$-dependance implies inter-association]
  \begin{gather}
    ...
    \begin{aligned}
      &\kinter{\verts}{Y,\rmZ \cup X}\\
      &:=
      \forall Z'\in\rmZ : \kassoc{\verts}{Y,Z'}{X \cup \rmZ
      \setminus Z'}
    \end{aligned}
  \end{gather}
  by contraction
...
\begin{lemma}[Subset]
  ...
  \begin{equation}
    \label{eq:22}
    \kdep{\verts}{Y,X}{\rmZ} \implies \kdep{\verts'}{Y,X}{\rmZ}
  \end{equation}
\end{lemma}
\begin{proof}
  Recall that by definition that
  \begin{align}
    \label{eq:5}
    \kdep{\verts}{Y,X}{\rmZ}\\
    \implies
      &\forall \rmZ' \subseteq \rmZ \:\:
    \forall \rmS \subset \verts \setminus \{Y,X\} : \\
      &Y \dep{\pr} X \mid \rmZ \cup \rmS
        \wedge Y \indep{\pr} X \mid \rmZ' \cup \rmS
  \end{align}
  \begin{align*}
...
\begin{algorithm}
  ...
  \Fn{make\_strict($Y$, $X$, $\rmX$, $\rmS$, $k$, $l$)}{
    ...
    %   \setminus X'$}
    \KwOut{$X\cup\rmX'$ s.t. $\rmX'\subseteq\rmX$ and $\kdep{\rmS}{Y,X}{\rmX'}$}
...
\begin{algorithm}
  ...
  \Fn{no\_seps($Y$, $X$, $\rmX'$, $\rmS'$, $l$, $\alpha$)}{
    ...
    \KwOut{is $Y \dep{} X \mid \rmX' \cup \rmS'$?}
    % max\_p $\gets 1$\;
    \For{$\rmZ \in \powerleq{l}{\rmS'}$}{
      % max\_p $\gets \min(\text{min\_p}, p)$\;
      \If{$Y \indep{} X \mid \rmX' \cup \rmZ$}{
        \Return{False}\;
      }
    }
    \Return{True}\;

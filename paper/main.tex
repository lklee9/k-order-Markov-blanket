...
\begin{definition}[$k$-Order Adjacency Faithfulness]
  ...
  \begin{equation}
    \label{eq:8}
    \exists \rmX \in [\mb(Y) \setminus X]^{\leq k-1}:
    Y \dep{\pr} X \mid \rmS \cup \rmX.
  \end{equation}
...
\begin{definition}[$k$-Order Dependence]
  ...
  \end{equation}
  We say that $Y$ and $X$ are associated w.r.t. $\rmZ$. Of course under
  this definition there might be some strict subset $\rmZ' \subset \rmZ$
  where $\kdep{Y,X}{\rmZ'}$ still holds. Therefore we say $Y$ and $X$
  are $|\rmZ|$-order dependent w.r.t $\rmZ$ only if this is not the
  case,
  \begin{gather}
    \begin{aligned}
      &\kdep{\rmV}{Y,X}{\rmZ}\\
...
\begin{algorithm}
  ...
  \Repeat(\tcp*[f]{While $\exists \rmX' \subseteq \rmV' \setminus \rmS : Y \strictdep{P} \rmX' \mid \rmS$}){$\rmX' = \emptyset$}{

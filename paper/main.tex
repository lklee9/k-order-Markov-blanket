...
\begin{tikzpicture}
  \node[draw, circle, inner sep=2pt] (1) at (-0.75,1.5) {$X_{1}$};
  % \node[draw, circle, inner sep=2pt] (2) at (0,1.5) {$X_{2}$};
  \node[draw, circle, inner sep=2pt] (3) at (0.75,1.5) {$X_{3}$};
...
\end{align*}
is the degrees of freedomn in the statistical test. A useful property of
the $\chi^{2}$ distribution that we will use later to prove
\Cref{thm:prune} is the fact that
...
It is well known that the G-statistic is propotional to the

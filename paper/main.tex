...
\begin{definition}[$k$-associations]
  Let $Y\in\rmV$ and $\rmX=\{X_{1},X_{2},\ldots,X_{k}\}\subset\rmV$ be
  ...
  \begin{equation}
    \label{eq:4}
    \forall X\in\rmX \;\vphantom{\in}\;
    \exists \rmZ \subseteq \rmX\setminus\{X\} :
    Y\assoc{|\rmZ|+1}\rmZ\cup\{X\}.
  \end{equation}
  The strict counterpart of this definition $Y\strict{\leq k}\rmX$
  has the additional property that $\forall X\in\rmX$ and for some $\rmZ
  \subset \rmX \setminus \{X\}$
  \begin{equation}
    \label{eq:6}
    \exists X \in \rmX : 
    \forall \rmZ \subset \rmX\setminus\{X\} :
    Y\notassoc{|\rmZ|+1}\rmZ\cup\{X\},
  \end{equation}
  or in other words, there's a variable $X\in\rmX$ that requires every
  other variable in $\rmX$ to be associated to $Y$.
\end{definition}
...
\begin{lemma}[Inverse Weak Union]
  ...
  $\rmX, \rmY, \rmZ \subset \rmV$ be disjoint subsets of $\rmV$. Then,
  $\forall \rmS \subseteq \rmZ \setminus (\rmX \cup \rmY \cup \rmZ)$
  \begin{equation}
    \label{eq:2}
...
\begin{lemma}[Inverse Decomposition]

...
violated in many ways and we will explore some of the ways it can be
violated in the following section.
...
Further note that when $k=1$, our notion of $k$-order dependence is
somewhat similar to \emph{$2$-association} in \citet[Definition
6]{marx2020}---but mainly with respect to being able to capture the
relationships between \emph{Unfaithful Triples} as defined in
\citet[Definition 5]{marx2020}.
...
- we need subset lemma
- but first
\begin{lemma}[$k$-Order Dependance Implies Inter-Association]\label{lem:k-dep-inter}
  \begin{gather}
    \label{eq:13}
    \kdep{\rmS}{Y,X}{\rmZ}
    \implies 

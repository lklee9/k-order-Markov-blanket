...
\begin{definition}[$k$-Order Dependence]
  ...
  \end{equation}
  Here we say that $Y$ and $X$ are associated w.r.t. $\rmZ$. Of course
  under this definition there might be some strict subset
  $\rmZ' \subset \rmZ$ where $\kdep{Y,X}{\rmZ'}$ still holds. Therefore
  we say $Y$ and $X$ are $|\rmZ|$-order dependent w.r.t $\rmZ$ only if
...
\begin{lemma}[$k$-dependance implies association]
  \begin{equation}
    \label{eq:13}
    \begin{aligned}
      &\kdep{\verts}{Y,X}{\rmX}\\
      ...
      %   \setminus X'\\
&\implies \forall X'\in\rmX : \kassoc{\verts}{Y,X'}{X \cup \rmX \setminus X'}
...
\begin{lemma}[Subset]
  Let $\verts' \subseteq \verts$ and $X,Y \in \verts$
\end{lemma}
...
\begin{algorithm}
  ...
  \Fn{make\_strict($Y$, $X$, $\rmX$, $\rmS$, $k$, $l$)}{
    ...
    %   \setminus X'$}
    \KwOut{$\rmX'\subseteq\rmX$ s.t. $\kdep{\rmS}{Y,X}{\rmX'}$}
    \While{$\exists X' \in \rmX : \neg$not\_ci($Y$, $X$, $\rmX
      \setminus X'$, $\rmS$, $k$, $l$)}{
      remove $X'$ from $\rmX$\;
    }
    \Return{$\rmX'$}\;

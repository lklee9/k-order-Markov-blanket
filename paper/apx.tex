...
\begin{proof}
  ...
  \end{equation}
  However, referring back to the definition of a $k$-order dependence in
  \Cref{def:k-dep}, we know that our initial assumption of
...
\begin{proof}
  ...
  variable $X$ that has not been included in $\rmX$ yet,
...
\begin{proof}
  ...
  appropriate $\rmZ$ such that $\linter{\rmS}{Y,X\cup\rmZ}$ is true and
  the set $X\cup\rmZ$ will be added into $\rmS$.

  Therefore once \textit{find\_inter\_dep} starts to return empty sets,
  we know that there are no more variables left in
...
\begin{table}[h]
  ...
  \caption{Minimal d-separator sizes restricted to the true Markov
    ...
    at most three.}
  \label{tab:dsep}
  \resizebox{\columnwidth}{!}{\input{./tab/dsep}}
\end{table}

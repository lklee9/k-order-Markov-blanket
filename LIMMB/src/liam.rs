...
pub fn nested_assoc_mine(
    ...
    while !stack_current.is_empty() {
        ...
        for i in 0..atts.len() {
            ...
            stack_prob_all.push(tmp_all_prob.clone());
            tmp_all_prob = tmp_all_prob.remove_att(a);
        }
    }
    return None;
}
...
pub fn prune(
    ...
    while !converged && cmb.len() > 0 {
        ...
        let mut reses: Vec<PruneRes> = Vec::with_capacity(cmb.len());
        for x in cmb.clone() {
            println!("\n\tpruning {}...", x);
            cmb.remove(&x);
            let res = prune_xs(
        ...
        let worse_res: &PruneRes = &reses[0];
        let x = worse_res.xs.last().unwrap();
        println!(
            "\t worse var: {}... nvals: {}, pval: {}, prune? {}",
            x,
            data.nvals[*x],
            worse_res.pval,
            worse_res.to_prune
        );
        if worse_res.to_prune {
            cmb.remove(x);
